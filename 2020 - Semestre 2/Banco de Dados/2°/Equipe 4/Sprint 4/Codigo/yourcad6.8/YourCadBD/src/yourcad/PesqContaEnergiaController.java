...
public class PesqContaEnergiaController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try { popular_tbView();} 
        catch (Exception ex) { Logger.getLogger(PesqContaEnergiaController.class.getName()).log(Level.SEVERE, null, ex);}       
       
        if("Digitador".equals(Form_LoginController.usuario_Nivel_Acesso))
        {
            //Funções não habilitadas para digitador: ******
            btn_alterarConta.setText("Visualizar");
            btn_deletarConta.setVisible(false);
            //***********************************************
        }
    }    
