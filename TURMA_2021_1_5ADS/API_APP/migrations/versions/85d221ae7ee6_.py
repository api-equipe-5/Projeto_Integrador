...
Revision ID: 85d221ae7ee6
Revises: 3472bbde4008
Create Date: 2021-05-15 02:06:22.797145
...
revision = '85d221ae7ee6'
down_revision = '3472bbde4008'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    op.add_column('customers', sa.Column('spc_raw_data', postgresql.UUID(as_uuid=True), nullable=True))
    op.drop_constraint('customers_raw_data_id_fkey', 'customers', type_='foreignkey')
    op.create_foreign_key(None, 'customers', 'spcrawdata', ['spc_raw_data'], ['id'])
    op.drop_column('customers', 'spc_raw_data_id')
...
def downgrade():
    ...
    op.alter_column('products', 'name',
               existing_type=sa.VARCHAR(length=255),
               nullable=False)
    op.add_column('customers', sa.Column('spc_raw_data_id', postgresql.UUID(), autoincrement=False, nullable=True))
